...
app.post('/register', upload.single('Idcard'), async (req, res) => {
    try {
        // Initialize DB on first request if needed (idempotent)
        await initDb();
        ...
        console.log(`A new registration for ${Personalinfo} has been inserted.`);
        // Redirect to success page
        res.redirect('/success.html');

    } catch (error) {
        console.error("Error during registration:", error);
        res.status(500).send("An error occurred during registration. Please check server logs.");
    }
});
